...
import { CreateTaskInput, TaskCategory } from "~/types/graphql";
...
const CreateTaskPanel = () => {
  ...
  const { register, getValues, reset, watch, setValue } =
    useForm<CreateTaskInput>({
      defaultValues: {
        leadIds: ctx.data.currentLeads?.map((l) => l.id),
      },
    });
  ...
  const createTask = async (e: FormEvent) => {
    ...
    if (response) {
      ...
      toast.error("Houve um erro ao criar a task.");
    }
  };
    ...
    <>
      <div className="mx-auto flex w-full max-w-[750px] flex-col gap-2 rounded-md border p-4">
        <div className="flex flex-row justify-between">
          <div className="text-3xl font-extrabold text-jpurple">Criar Task</div>
        </div>

        <form className="mt-12 flex flex-col gap-4" onSubmit={createTask}>
