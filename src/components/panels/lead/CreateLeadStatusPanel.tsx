...
import { Category, CreateLeadStatusInput } from "~/types/graphql";
...
const CreateLeadStatusPanel = () => {
  const { data: session } = useSession();
  const router = useRouter();
  const ctx = useContext(reactContext);

  const { register, getValues, reset, watch } =
    useForm<CreateLeadStatusInput>();
  ...
  const createLeadStatus = async (e: FormEvent) => {
    e.preventDefault();
    ...
    const response = await fetchData({
      mutation: `
        mutation {
        createLeadStatus(createLeadStatusInput: {
        name: "${name}"
        color: "${color}"
    ...
    if (response) {
      ...
      toast.error("Houve um erro ao criar o Status.");
    }
  };
  ...
  return (
    <>
      <div className="mx-auto flex w-full max-w-[750px] flex-col gap-2 rounded-md border p-4">
        ...
        <form className="mt-12 flex flex-col gap-4" onSubmit={createLeadStatus}>
          ...
          <div className="mt-8 flex flex-row items-center justify-end gap-4">
            <button
              ...
              onClick={() => handlePanelChange("leads-status")}
            >
              Voltar
            </button>
...
export default CreateLeadStatusPanel;
